	...
	public class CustomTableModel extends AbstractTableModel {
	    ...
	    public Object getValueAt(int rowIndex, int columnIndex) {
	        if (columnIndex == 6 || columnIndex == 7) {
	            return data.get(rowIndex)[columnIndex]; // La valeur de la cellule est d�j� une instance d'ImageUtility
	        } else {
	    ...
	    public void updateRow(int rowIndex, Object[] newData) {
	        if (rowIndex >= 0 && rowIndex < data.size()) {
	            for (int columnIndex = 0; columnIndex < newData.length; columnIndex++) {
	                if (columnIndex == 6 || columnIndex == 7) {
	                    // Si la colonne contient un ImageUtility, assurez-vous que la nouvelle donn�e est �galement un ImageUtility
	                    if (newData[columnIndex] instanceof ImageUtility) {
	                        data.get(rowIndex)[columnIndex] = newData[columnIndex];
	                    }
	                } else {
	                    // Pour les autres colonnes, remplacez simplement les donn�es
	                    data.get(rowIndex)[columnIndex] = newData[columnIndex];
	                }
	            }
